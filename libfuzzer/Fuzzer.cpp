                       ...
                       const tuple<unordered_set<uint64_t>, unordered_set<uint64_t>> &validJumpis)
{
    ...
    expFile << "time: " + to_string(duration) + ",all patterns:" + to_string(patterns.size()) << endl;
...
void Fuzzer::stop(const tuple<unordered_set<uint64_t>, unordered_set<uint64_t>> &validJumpis)
{
    ...
    expFile << "uncovered Patterns: " + to_string(uncoveredPatterns.size()) << endl;
    expFile << "gasless send: " + string(toResult(vulnerabilities[GASLESS_SEND])) << endl;
    expFile << "dangerous delegatecall: " + string(toResult(vulnerabilities[DELEGATE_CALL])) << endl;
    expFile << "exception disorder: " + string(toResult(vulnerabilities[EXCEPTION_DISORDER])) << endl;
    expFile << "freezing ether: " + string(toResult(vulnerabilities[FREEZING])) << endl;
    expFile << "reentrancy: " + string(toResult(vulnerabilities[REENTRANCY])) << endl;
    expFile << "integer overflow: " + string(toResult(vulnerabilities[OVERFLOW])) << endl;
    expFile << "timestamp dependency: " + string(toResult(vulnerabilities[TIME_DEPENDENCY])) << endl;
    expFile << "integer underflow: " + string(toResult(vulnerabilities[UNDERFLOW])) << endl;
    expFile << "block number dependency: " + string(toResult(vulnerabilities[NUMBER_DEPENDENCY])) << endl;
...
void Fuzzer::start()
{
    ...
    for (auto contractInfo : fuzzParam.contractInfo)
    {
        ...
        else
        {
            ...
            expFile = ofstream(contractName + ".txt", ios_base::app);
